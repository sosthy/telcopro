...
{
	@Query("select r from RecipientGroupe r inner join r.recipients rr where lower(r.name) like lower(:x) or "
			+ "lower(rr.name) like lower(:x) or lower(rr.surname) like lower(:x)")
